...
import { useEditTransaction } from '~/api/transactionsAPI'
...
export const EditTransactionModal = component$(
  ({ account, transaction, close }: EditTransactionModalProps) => {
    ...
    const state = useStore({
      categoryId: categories.find((c) => c.id === transaction.categoryId)?.id,
    ...
    const handleConfirm = $(async () => {
      ...
      await editTransaction.submit(newTransaction)
      close()
    })
        ...
        <Modal title="Nowe konto" handleConfirm={handleConfirm} handleCancel={handleCancel}>
          ...
          <div class="flex flex-col gap-1">
            ...
            <select
              value={state.status}
              onChange$={(e) =>
                (state.status = (e.target as HTMLSelectElement).value as TransactionStatus)
              }
            >
              {statuses.map((c) => (
